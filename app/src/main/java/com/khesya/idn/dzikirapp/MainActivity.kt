package com.khesya.idn.dzikirapp
...
import com.khesya.idn.dzikirapp.UI.QauliyahActivity
...
class MainActivity : AppCompatActivity() {
    ...
    private val slidingCallBack = object : ViewPager2.OnPageChangeCallback(){
        override fun onPageSelected(position: Int) {
            for (dot in 0 until dotsCount) {
                dotSlider[dot]?.setImageDrawable(ContextCompat.getDrawable(
    ...
    private fun setUpViewPager() {
        ...
        for (i in 0 until dotsCount) {
            ...
            dotSlider[i]?.setImageDrawable(
                ContextCompat
                    .getDrawable(applicationContext, R.drawable.non_active_dot)
            )
    ...
    private fun initView() {
        ...
        vpArtikel = findViewById(R.id.vpArttikel)
    }
}